...
float vertexData[] = {
    ...
    -1.f, .0f,  1.f,
     1.f, .0f, -1.f};
GLuint numVertices = sizeof(vertexData) / sizeof(*vertexData);
}

WorldDrawable::WorldDrawable()
: m_isComplete(false)
, m_vertexBuffer(-1)
, m_vertices(std::vector<float>(onlyForTesting::vertexData, onlyForTesting::vertexData+onlyForTesting::numVertices))
, m_program(nullptr)
{
...
void WorldDrawable::drawImplementation(osg::RenderInfo & renderInfo) const
{
    ...
    state->glDrawArraysInstanced(GL_TRIANGLE_STRIP, 0, m_vertices.size()/3, 1);
...
void WorldDrawable::initialize(osg::RenderInfo & renderInfo)
{
    ...
    ext->glBindBuffer(GL_ARRAY_BUFFER_ARB, m_vertexBuffer);
    ext->glBufferData(GL_ARRAY_BUFFER_ARB, m_vertices.size(), m_vertices.data(), GL_STATIC_DRAW);
...
void WorldDrawable::reloadShaders(osg::State & state)
{
    ...
    m_program->compileGLObjects(state);
}

osg::BoundingBox WorldDrawable::computeBound() const
{
    return osg::BoundingBox(osg::Vec3(-1.f, -1.f, -1.f), osg::Vec3(1.0f, 1.0f, 1.0f));
}
